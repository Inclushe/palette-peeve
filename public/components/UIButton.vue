<template>
  <a @click="runAction($event)" :class="{button: true, 'button--disabled': buttondisabled || activewhenselecting && currentlySelectedShade === null, 'button--mini': type === 'mini'}" role="button" @mousedown="rippleStart" @mouseup="rippleEnd" v-on:click.stop @mouseover.stop="hoverStart" @mouseout.stop="hoverEnd" data-ripple="true" :data-action="action" :data-tooltip-info="name" :data-tooltip-keys="keyboardshortcut">
